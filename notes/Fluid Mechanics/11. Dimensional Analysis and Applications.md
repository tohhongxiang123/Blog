...
- At low $Re$, viscous force is dominant over inertia force, laminar flow occurs
- At high $Re$, inertia force is dominant over viscous force, turbulent flow occurs

## Derivation of Reynold's Number

Consider a tube of small area $A$, through which a fluid of density $\rho$ is flowing with velocity $v$

The mass of fluid through the tube per second,

$$
\Delta m = \text{Volume of fluid flowing per second $\times$ density} = AV \rho
$$

The inertial force per unit area is the rate of change of momentum per unit area
...
$$
\tau = \frac{F}{A} = \mu \frac{V}{r}
$$

where $r$ is the radius of the tube, and $\frac{V}{r}$ is the velocity gradient between the layers of the liquid flow

Since Reynold's number is the inertial force per unit area / viscous force per unit area,

$$
Re = \frac{V^2 \rho}{\mu \frac{V}{r}} = \frac{V \rho r}{\mu}
$$
...
$Fr$ is important in flows with a free surface where gravity is important, such as open channel flows
...
Weber's number is important when there is an interface between 2 fluids, and surface tension may be important
