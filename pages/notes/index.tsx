...
export default function NotesPage({
    ...
    const shownNotes = notes.filter((note) =>
        note.data.title.toLowerCase().includes(searchText.toLowerCase())
    );
    return (
        <NotesLayout
            title="Notes"
            notesStructure={notesStructure}
            description="All notes written by Toh Hong Xiang"
        >
            <div className="p-4 max-w-2xl mx-auto flex flex-col">
                ...
                <Link
                    href="/"
                    className="font-medium rounded-md py-2 px-4 hover:bg-gray-100 ml-auto"
                >
                    Back
                </Link>
            </div>
