...
export const getFilesInDirectory = (directory: string) =>
    fs
...
export function recursivelyGetFilesInDirectory(directory: string) {
    ...
    currentFolderResults.forEach((fileOrFolderName) => {
        if (
            fs
                .statSync(path.join(currentFullPath, fileOrFolderName))
                .isDirectory()
        ) {
            results.push(
                ...recursivelyGetFilesInDirectory(
                    [directory, fileOrFolderName].join('/')
                )
            );
        } else {
...
function getLastModified(directory: string) {
    ...
    const lastModifiedTime = Math.max(
        directoryStats.birthtime.getTime(),
        ...fs
            .readdirSync(directory)
            .map((fileOrFolderName) =>
                new Date(
                    getLastModified(path.join(directory, fileOrFolderName))
                ).getTime()
            )
    );
...
export function getFilesWithStructure(directory: string) {
    ...
    const currentFileName = directory.replace(/\\/g, path.sep).split(path.sep)[
        directory.replace(/\\/g, path.sep).split(path.sep).length - 1
    ];
    ...
    if (fs.statSync(path.join(currentFullPath)).isDirectory()) {
        result.children = fs
            ...
            .sort(collator.compare)
            .map((fileOrFolderName) =>
                getFilesWithStructure(path.join(directory, fileOrFolderName))
            );
    }
...
export async function renderContentWithPlugins({
    content,
    data
}: {
    content: string;
